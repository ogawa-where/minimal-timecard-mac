...
> **Note:** 署名なしアプリのため、初回起動時に Gatekeeper でブロックされることがあります。その場合は [Troubleshooting](#troubleshooting) を参照してください。
...
### 起動できない / 「ゴミ箱に入れる」と表示される

署名なしアプリのため、Gatekeeper の quarantine 属性により起動がブロックされることがあります。
...
```bash
# アプリ自身の非表示記録を削除（残っているとアプリが起動直後に終了します）
defaults delete com.ogawa-where.MinimalTimecard "NSStatusItem VisibleCC Item-0"

# アプリを起動
open -a MinimalTimecard
```

それでもアイコンが表示されない場合は、System Settings → Control Center → 「Menu Bar Only」セクションから `MinimalTimecard` を探し、「Show in Menu Bar」に切り替えてください。
